
# Devflow OS

### A Modern Browser-Based Operating System Experience

Lovable OS is a fully interactive desktop environment built entirely with **HTML5, CSS3, and Vanilla JavaScript**. It recreates the feel of a modern operating system directly in the browser, featuring window management, desktop widgets, application launching, theming, notifications, and a polished user experience.

Designed as a frontend engineering showcase, the project demonstrates advanced DOM manipulation, modular architecture, responsive design, accessibility practices, and performance-focused UI development without relying on any frameworks.

---

## 🚀 Live Demo

**Visit the application:**

👉 https://devflowos.netlify.app/

---

## 📸 Preview


* Desktop Environment
* Window Management
* Start Menu
* Notification Center
* Spotlight Search
* Themes & Personalization

---

## ✨ Features

### Desktop Environment

* Interactive desktop interface
* Desktop icons
* Context menus
* Smooth transitions
* Responsive layout

### Window Management System

* Open multiple applications
* Drag and move windows
* Resize windows
* Minimize / Maximize / Restore
* Window focus management
* Dynamic z-index handling

### Start Menu

* Application launcher
* Search functionality
* Quick access controls
* Modern operating-system style UI

### Spotlight Search

* Global application search
* Instant search results
* Keyboard-first navigation

### Notification System

* Animated toast notifications
* Notification center
* Real-time alerts
* Dismiss and clear actions

### Theme Engine

* Light Theme
* Dark Theme
* Midnight Theme
* Cyber Theme

Theme preferences are automatically persisted using Local Storage.

### Personalization

* 8 built-in wallpapers
* Theme customization
* Persistent settings

### Built-In Applications

#### 📁 File Explorer

* Folder navigation
* File organization interface

#### 📝 Notes

* Create notes
* Edit notes
* Persistent local storage

#### 🌐 Browser

* Embedded browsing experience

#### 💻 Terminal

* Simulated command-line environment
* Interactive commands

#### 🧮 Calculator

* Standard calculator functions

#### ⚙️ Settings

* Theme management
* Wallpaper management
* System customization

### Desktop Widgets

* Clock Widget
* Weather Widget
* Calendar Widget
* Quick Notes Widget

### Accessibility

* Semantic HTML
* Keyboard navigation
* ARIA labels
* Focus management

### Responsive Design

Optimized for:

* Desktop (1920px+)
* Laptop (1366px+)
* Tablet (768px+)
* Mobile (375px+)

---

## ⌨️ Keyboard Shortcuts

| Shortcut | Action                |
| -------- | --------------------- |
| Ctrl + K | Open Spotlight Search |
| Ctrl + , | Open Settings         |
| Esc      | Close Active Popup    |

*(Cmd key supported on macOS.)*

---

## 🛠️ Tech Stack

* HTML5
* CSS3
* Vanilla JavaScript (ES Modules)
* Local Storage API

No frameworks. No libraries. No build tools.

---

## 📂 Project Structure

```text
/
├── index.html
├── css/
│   ├── styles.css
│   ├── themes.css
│   └── components.css
├── js/
│   ├── app.js
│   ├── windowManager.js
│   ├── notificationManager.js
│   ├── searchManager.js
│   ├── themeManager.js
│   └── apps/
├── assets/
│   ├── wallpapers/
│   ├── icons/
│   └── images/
└── README.md
```

---

## ⚡ Performance Highlights

* Optimized DOM updates
* Event delegation
* Hardware-accelerated animations
* Lightweight architecture
* Fast startup experience
* Responsive interactions
* Local-first data persistence

---

## 🎯 What This Project Demonstrates

* Advanced JavaScript Architecture
* Complex UI State Management
* Window Management Systems
* Responsive Web Design
* Accessibility Best Practices
* Browser Storage Techniques
* Frontend Performance Optimization
* Real-World Product Design

---

## 🌍 Deployment

The project can be deployed on:

* [Vercel](https://vercel.com?utm_source=chatgpt.com)
* [Netlify](https://www.netlify.com?utm_source=chatgpt.com)
* [GitHub Pages](https://pages.github.com?utm_source=chatgpt.com)

---

## 📄 Resume Description

> Developed **Devflow OS**, a browser-based operating system featuring multi-window management, desktop widgets, application launching, notification systems, theme personalization, and responsive user interfaces using Vanilla JavaScript, HTML5, and CSS3.

---

## 👨‍💻 Author

**Vikash Jha**

GitHub: jhavikash01

---

**Tip:** Deploy it on Vercel and put the actual URL in the **Live Demo** section at the top. Recruiters usually click the demo link before reading the code.
